"""v66: Answer-based hit 기준 검색 평가.

v48(mz-RAG)과 동일한 평가 방식: 리트리브된 세그먼트에 정답 텍스트가 포함되어야 hit.
기존 retrieval.py는 gold doc의 아무 세그먼트면 hit (doc-based) → 과대평가 문제.

실행: python -m eval.retrieval_v66 --checkpoint runs/.../best.pt [--config ...]
"""
...
def main():
    ...
    output_dir = Path("experiments/v66_answer_hit/z_retrieval")
    ...
    print(f"\n[v66 z-retrieval] MRR={metrics['mrr']:.4f}", end="")
    for k in top_k:
